...
CONFIG_PATH = Path(
    __file__).resolve().parent / "config" / "parasocial" / "en.yaml"


def calculate_parasocial_impact(channel_handle: str,
    ...
    if not csv_path.exists():
        raise FileNotFoundError(
            f"Processed CSV not found at {csv_path}. Run Pillar 1 first.")
    ...
    avg_word_count = round(float(df["word_count"].mean()),
                           2) if total_comments else 0.0
    word_count_std = round(float(df["word_count"].std()),
                           2) if total_comments > 1 else 0.0
    longform_comments_pct = round(
        (len(df[df["word_count"] >= 50]) / total_comments) *
        100, 2) if total_comments else 0.0
    ...
    baseline_avg_score = round(float(df["parasocial_score"].mean()),
                               4) if total_comments else 0.0
    moderate_or_higher = df[df["parasocial_score"] >=
                            config.thresholds["moderate"]]
    strong_or_higher = df[df["parasocial_score"] >=
                          config.thresholds["strong"]]
    very_strong = df[df["parasocial_score"] >=
                     config.thresholds["very_strong"]]

    parasocial_count = len(moderate_or_higher)
    parasocial_density = round((parasocial_count / total_comments) *
                               1000, 2) if total_comments else 0.0
    ...
    if "video_id" in df.columns and "like_count" in df.columns:
        ...
        if total_top > 0:
            ...
            top_moderate = top_df[top_df["parasocial_score"] >=
                                  config.thresholds["moderate"]]
            top_moderate_count = len(top_moderate)
            top_parasocial_density = round(
                (top_moderate_count / total_top) * 1000, 2)

            for cats_str in top_df["parasocial_categories"].dropna():
                if cats_str:
                    for cat in cats_str.split(", "):
                        if cat:
                            top_category_counts[cat] = top_category_counts.get(
                                cat, 0) + 1
    ...
    strongest_samples = sorted_df[sorted_df["parasocial_score"] >=
                                  config.thresholds["strong"]].head(3).to_dict(
                                      "records")
    moderate_samples = sorted_df[
        (sorted_df["parasocial_score"] >= config.thresholds["moderate"])
        & (sorted_df["parasocial_score"] < config.thresholds["strong"])].head(
            3).to_dict("records")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description=
        "Run Pillar 3 Parasocial Signal Detector on processed channel data.")
    parser.add_argument(
        "handle",
        nargs="?",
        help=
        "Channel handle (e.g. donflurgundy). If omitted, processes all CSVs in data/processed/."
    )
    ...
    if args.handle:
        ...
        try:
            ...
            print(
                f"Baseline Avg Parasocial Score: {report['baseline']['avg_parasocial_score']}"
            )
            print(
                f"Top-Comment Avg Parasocial Score: {report['top_comments']['avg_parasocial_score']}"
            )
            print(
                f"Resonance Parasocial Delta (ΔR_parasocial): {report['resonance_delta_parasocial']:+} points"
            )
            print(
                f"Top-Comment Parasocial Density: {report['top_comments']['parasocial_density_per_1k']} / 1,000 comments"
            )
        ...
        if not csv_files:
            print(
                "[!] No processed CSV files found in data/processed/. Run Pillar 1 first."
            )
