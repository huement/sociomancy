...
def process_empathy_and_toxicity(json_path: Path, top_n_per_video: int = 1) -> dict:
    ...
    uncached_comments = [c for c in raw_comments if str(c["comment_id"]) not in cached_comment_ids]

    if uncached_comments:
        print(f"[*] Found {len(uncached_comments)} new comments requiring ONNX inference...")
        ...
        new_df["top_emotion"] = [e["top_emotions"][0]["label"] if e["top_emotions"] else "neutral" for e in evaluations]
        ...
        print(f"[*] [CACHE HIT] All {len(raw_comments)} comments already processed in CSV. Skipping ONNX inference.")
        df = existing_df
    ...
    toxicity_pct = round((toxic_count / total_comments) * 100, 2) if total_comments > 0 else 0.0
    avg_toxicity = round(float(df["toxicity_score"].mean()), 4) if total_comments > 0 else 0.0
    ...
    top_toxicity_pct = round((top_toxic_count / total_top_comments) * 100, 2) if total_top_comments > 0 else 0.0
    top_avg_toxicity = round(float(top_df["toxicity_score"].mean()), 4) if total_top_comments > 0 else 0.0
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run Pillar 1 ONNX Empathy/Toxicity model on cached raw JSON.")
    parser.add_argument("handle", nargs="?", help="Channel handle (e.g., donflurgundy). If omitted, processes all raw files in data/raw/.")
    ...
    if args.handle:
        ...
        if target_file.exists():
            ...
            print(f"Baseline Toxicity: {res['baseline']['toxicity_percentage']}% (Avg: {res['baseline']['average_toxicity_score']})")
            print(f"Top-Comment Toxicity: {res['top_comments']['toxicity_percentage']}% (Avg: {res['top_comments']['average_toxicity_score']})")
            print(f"Resonance Toxicity Delta (ΔR_tox): {res['resonance_delta_toxicity']:+} points")
            print(f"Top-Comment Emotions: {res['top_comments']['dominant_emotions']}")
        else:
        ...
        for f in raw_files:
            ...
            print(f"Resonance Toxicity Delta (ΔR_tox): {res.get('resonance_delta_toxicity', 0.0):+} points")